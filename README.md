...
- `/setup_panel <channel>` - 上記コマンドをカテゴリ別のリストボックス(基本/音楽制作補助/情報共有・進行管理/告知・日程調整)から選んで実行できる常設パネルを設置する(管理者用)。`/`コマンドの使い方がわからないメンバー向けの導線。パネルからは自分自身に対する操作(メンバー指定・ロール通知なしの簡易版)のみ行え、他メンバー指定やロール通知など詳細な引数を使いたい場合は引き続き通常のスラッシュコマンドを使う。
